...
The existing ``test_lingpai.py`` e2e cases use a single project posing as
...
- ``bob`` — sender (signs messages with his keypair; ``alice``'s
  ``bangjiao.yaml`` carries his public key)
...
def two_courts(tmp_path, monkeypatch):
    """Set up alice + bob with cross-signed bangjiao.yaml entries.
    ...
    _seed_project(root, "alice", allow_paths=["bus/zongguan/inbox/**"])
    _seed_project(root, "bob", allow_paths=["bus/zongguan/inbox/**"])

    alice_id = bangjiao.generate_keypair("alice", force=True)
    bob_id = bangjiao.generate_keypair("bob", force=True)

    # Alice trusts Bob (tier_a so we can see grants flip the decision).
    bangjiao.project_peers_yaml_path("alice").write_text(yaml.safe_dump({
        "peers": [{
            "name": "Bob's court",
            "yamen_id": "bob",
    ...
    bangjiao.project_peers_yaml_path("bob").write_text(yaml.safe_dump({
        "peers": [{
            "name": "Alice's court",
            "yamen_id": "alice",
            "url": "http://127.0.0.1:0",
...
def _signed_from_bob(bob_id, *, attaches=None, body="hi"):
    ...
    msg["signature"] = bangjiao.sign_message(msg, bob_id.priv)
    return msg
...
def test_path_grant_walkthrough(two_courts):
    ...
    g = lingpai.mint_path_grant("alice", "bob", ["notes/**"], ttl="1h")
    ...
    after = lingpai.find_grant("alice", g.id)
...
def test_path_grant_walkthrough_tier_c_peer(tmp_path, monkeypatch):
    ...
    assert lingpai.revoke_grant("alice", g.id) == "revoked"
...
def test_tier_grant_consume_on_use_walkthrough(two_courts):
    ...
    g = lingpai.mint_tier_grant(
        "alice", "bob", "tier_c", ttl="1h", consume_on_use=True,
    )

    msg1 = _signed_from_bob(bob_id, attaches=["bus/zongguan/inbox/x.md"])
    _, b1 = _round_trip("alice", msg1)
    ...
    msg2 = _signed_from_bob(bob_id, attaches=["bus/zongguan/inbox/y.md"])
    _, b2 = _round_trip("alice", msg2)
