"""agent-yamen — generate a project's identity keypair.

Invoked by ``bin/court-keygen <project>``. Writes to
``$YAMEN_ROOT/projects/<project>/identity/``.
...
def main() -> int:
    ...
    if project_priv_key_path(project).exists() and not args.force:
        ...
        print(f"both into their bangjiao.yaml under the entry for project '{project}'.")
        return 0
    ...
    print("them into the entry for this yamen_id in their bangjiao.yaml.")
    return 0
