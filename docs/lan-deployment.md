...
把两个 `agent-yamen` project 通过同一个局域网连起来。无需公网 IP，无需 VPN ——
只要两台机器能在 LAN 上互通就能跑。

> 状态：PR-1（HTTP + 签名 + 角色白名单）、PR-2（策略引擎 + 路径/关键字门禁
> + pending-approval 桶）、PR-3（LLM 裁判 + 失败安全回落）、PR-4
> （sudo 风格临时授权，走 `banling`）都已上线。还没做：PR-5 多通道
> 人工审批（飞书 / 微信）、PR-6 IM 冗余、TLS。

## 先建立心智模型

一座"法庭"住在**某台机器上的某个 project** ——
`$YAMEN_ROOT/projects/<project>/`。每个 project 有自己的密钥对、自己的
`bangjiao.yaml`、自己的 `yamen_id`。同一台机器上的两个 project 互相**无法**
...
1. `agent-yamen` 已经 checkout，`bin/` 在 PATH 上。
2. MCP server 的 venv 已经装好：
   ...
   ```
3. 你想联邦的 project 已经存在于 `$YAMEN_ROOT/projects/<project>/`。
   仓库里的示例 project 直接可用：
   ```bash
...
再跑一次 `zhuyin example` 是 no-op，除非加 `--force`。

## 2. 在 `yamen.yaml` 里启用联邦

默认 `yamen.yaml` 的 `bangjiao:` 块是注释掉的 —— 守护进程拒绝启动。
取消注释（或自己写一份），配好白名单：
...
bangjiao:
  ...
  # 你在网络上的 yamen_id。默认 "<hostname>-<project>"。
  yamen_id: "alice-laptop-example"

  # 外部 peer 可以派给哪些 role。默认仅 zongguan。
  expose_roles:
    - zongguan
...
| 字段 | 要发的值 |
|---|---|
| `yamen_id` | 对方会用什么名字引用你。默认 `<hostname>-<project>`；在 `yamen.yaml` 的 `bangjiao.yamen_id` 里覆盖。 |
| `fingerprint` | `zhuyin` 输出的 16 字节 hex。让对方第一次粘公钥时能用眼睛核对一遍。 |
| `pub_key_b64` | 完整的 base64 公钥（`zhuyin` 输出里也有；或者 `cat $YAMEN_ROOT/projects/<project>/identity/pub.key`）。**运行时必需** —— 没有它对方无法校验你的签名。 |

## 4. 每边各写一份 `bangjiao.yaml`

这个文件**住在 project 里面**，不是共享配置目录里。

Alice 的 `~/.agent-yamen/projects/example/bangjiao.yaml`：
...
Bob 的 `~/.agent-yamen/projects/example/bangjiao.yaml` —— 对称，列出 Alice
...
`http://host:8766` 等）；远端在他们 `bangjiao.yaml` 里**对该 project**
的条目就用对应的 URL。
...
如果那个 project 的 `yamen.yaml` 关闭了联邦，守护进程会拒绝启动，
并指向配置块的位置提示你。

## 6. 从 Alice → Bob 发一条测试消息

从任何接到 Alice 的 `yamen-mcp` 的 MCP 客户端（Claude Code、Cursor、Zed、
自研助手）调：

```python
lie_fanbang(project="example")
# 返回：{project, self: {yamen_id, fingerprint, bangjiao_enabled, ...},
...
Bob 目前还得**手动**把文件投给他的 zongguan —— `qijuguan` 守护进程
...
# 把任何想投给 zongguan 的文件提升：
mv .../bus/<peer-yamen-id>/inbox/<file>.md \
   .../bus/zongguan/inbox/<file>.md
```

后续 PR 会教 `qijuguan` 在策略判定是 `auto_pass` 时也自动把
...
| 判定 | 落到 | 含义 |
|---|---|---|
| `auto_pass` / `judge` | `bus/<peer>/inbox/` | 正常投递给 zongguan |
| `human_required` | `bus/<peer>/pending-approval/` | 等审批 —— 得人手动 `mv` 到 inbox |
| `denied` | `bus/<peer>/denied/` | 仅审计；永远不到 zongguan |

`guoshu_fanbang` 的响应里始终带判定结果，发送方 LLM 可以据此反应：
...
加一份 `~/.agent-yamen/projects/example/lvli.yaml` 来调默认 tier
和加自定义敏感词：
...
（或者 `yamen.yaml` 里的 `default_cli`）和内建 prompt
...
失败原因保留在 `panduo.jsonl` 的 `reasons` 数组里 —— 收到可疑投递
后 `tail` 一下。
...
审查，他们就发一张授权而不是改 `yamen.yaml`。授权是**时效绑定 + peer
...
banling example bob --tier tier_c --once          # 一次免审
banling example bob --tier tier_c --ttl 1h        # 限时信任窗口
...
每张授权是 `$YAMEN_ROOT/projects/<p>/grants/<id>.json` 下的一个 JSON
...
| `granted_to` | string | Peer `yamen_id`。必须匹配入站消息的 `from_court`。 |
...
- **路径包容性**。每个授权入口都校验 `project` 是单一安全文件系统组件，
  并且解析后必须严格位于 `$YAMEN_ROOT/projects/` 之下。传
...
### 按 peer 设 tier（在 `bangjiao.yaml` 里）
...
guoshu_fanbang(
    ...
    attaches=["bus/zongguan/inbox/diff.md"],   # 在 allow_paths 内
)
...
mv pending-approval/<file>.md inbox/   # 放出去给 zongguan
```

## 防火墙清单

`tongzheng` 是纯 HTTP，没有 TLS。每台机器双向放行那个端口 —— 大多数
家庭 LAN 本来就敞着。

| 操作系统 | 放行入站 TCP 8765 |
|---|---|
| macOS | `系统设置 → 网络 → 防火墙 → 选项 → 给 tongzheng 的 python 二进制设 "允许"` |
...
### `guoshu_fanbang` 响应里出现 `transport_error`
...
- 对端拒了你的签名。基本是这几种：
  - 对端 project 的 `bangjiao.yaml` 里你 `yamen_id` 对应的 `pub_key_b64`
    和你当前的 `priv.key` 不匹配。重新跑过 `zhuyin`？得把新公钥
    重新分给对方。
  - 你和对端对"签名 payload 包含哪些字段"看法不一致。两边必须是同一个
    `agent-yamen` 版本。
  - 你 `guoshu_fanbang` 的 `project=...` 指错了，用来签名的私钥
    属于另一个 court，不是对方期待的那个。
- 看接收端的 `$YAMEN_ROOT/projects/<project>/logs/bangjiao-errors.log`
  看具体失败原因。

### 403 `bangjiao_disabled`

- 对端 `yamen.yaml` 没有 `bangjiao:` 块，或者 `bangjiao.enabled:
  false`。这个 flag 是按请求重读的，所以对端把它翻回 `true` **无需**
  重启守护进程。

### 403 `unknown_sender`

- 你的 `yamen_id` 不在对端 `bangjiao.yaml` 里。让对方加上，或者检查你
  用的 `yamen_id` 和他们配的一致。记住：**每个 project 有自己的
  `bangjiao.yaml`** —— 被列在他们 `project-A/bangjiao.yaml` 里并不能让你
  访问 `project-B`。

### 403 `role_not_exposed`

- 你派给了一个不在对端 `bangjiao.expose_roles` 里的 role。默认只
  暴露 `zongguan`；要么走 zongguan，要么让对方把目标 role 加进
  `expose_roles`。

### 响应里 `decision: denied`

- 某个 attach 命中了 deny 规则（你的或硬编码的）。消息**没**投递 ——
  它坐在接收端的 `bus/<your-yamen-id>/denied/` 里仅供审计。看响应的
  `reasons` 字段：
  ```
  "reasons": ["attach '/etc/passwd' hits hardcoded deny '/etc/**'"]
  ```
- 硬编码 deny **既不能**通过 `yamen.yaml` 解锁，**也不能**通过 PR-4
  授权解锁 —— 设计如此。如果你确实需要那条路径，重构这次派发（比如
  把相关内容粘进消息正文）。如果只是被你自己的
  `allow_paths`/`deny_paths` 挡了，让对方铸造临时授权：
  `banling <project> <your-yamen-id> "<glob>"`（见上面的"临时授权"）。
...
- 要么发送方的 peer 条目是 `policy_tier: tier_a`，要么正文触发了
  ...
  `bus/<your-yamen-id>/pending-approval/` 里；得人手动 `mv` 到
  `inbox/` 才算投递。
- 看接收端 `logs/panduo.jsonl` —— 每条判定都有 `reasons` 数组
  解释哪条规则触发了。
...
- PR-3 裁判尝试调 LLM CLI 失败了。看消息的 `reasons` 数组 —— 它给出
  ...
  - `"cli '<x>' not found on PATH"` → 接收端机器装上那个 CLI，或者
    把 `bangjiao.judge.cli` 指向一个存在的。
  - `"<x> timed out after Ns"` → CLI 跑太慢。要么调高
    `bangjiao.judge.timeout_seconds`，要么换用 `judge.model`
    指更快的模型。
...
### `lie_fanbang` 显示 `reachable: false`
...
- **推荐**：两台机器都装 [tailscale](https://tailscale.com)，在
  `bangjiao.yaml` 里用 tailscale 分配的 IP。从这往后和 LAN 一样，还附带
  端到端加密。
- **自托管**：用 `frp` 或 `cloudflared` 把 tongzheng 端口暴露出去。
  公网 URL 写进 `bangjiao.yaml`。如果流量真的穿越公网，配上真正的 TLS
  反向代理（PR-1 不发 TLS）。
