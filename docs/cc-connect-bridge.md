...
`chizhao_zongguan`）→ yamen-mcp 写文件 → qijuguan 路由 →
对应 role 干活 → 结果最终通过 `lan_chengzou` 回来。
...
不管用哪种 agent，你都得自己负责让它配上 `agent-yamen` MCP server。
具体怎么配看各 agent 的文档。
...
你已经接入了 `agent-yamen` MCP server。当用户想让某个本地 court 干活时，
调用它的工具：

| 用户说 | 调用 |
|---|---|
| "我有哪些项目" | `lie_yamen()` |
| "把 X 发给 <project> 的 zongguan" | `chizhao_zongguan(project, message)` |
| "把 X 发给 <project> 的 backend" | `chizhao_zongguan(project, message, target_role="backend")` |
| "<project> 那边怎么样了" | `tang_yamen(project)` |
| "<project> 有回信吗" | `lan_chengzou(project)` |

如果不确定该用哪个 project，先调 `lie_yamen()` 看一眼。
```
...
- 有些桥会把工具调用 + 工具结果当成中间消息直接抖到聊天界面里。
  这是桥的 UI 问题，不是 agent-yamen 的行为。如果平台允许，配置桥
  把中间事件抑制掉就行。
